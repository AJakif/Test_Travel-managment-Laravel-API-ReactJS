<div id="notifications">
    <a class="nav-link dropdown-toggle" href="#" id="alertsDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
        ...
        <span class="badge badge-danger badge-counter">
            @if(count(session('Loggeduser')->unreadNotifications) >5 )<span data-count="5" class="count">5+</span>
            @else 
                <span class="count" data-count="{{count(session('Loggeduser')->unreadNotifications)}}">{{count(session('Loggeduser')->unreadNotifications)}}</span>
            @endif
        </span>
      </a>
      <!-- Dropdown - Alerts -->
      <div class="dropdown-list dropdown-menu dropdown-menu-right shadow animated--grow-in" aria-labelledby="alertsDropdown">
        ...
        @foreach(session('Loggeduser')->unreadNotifications as $notification)
